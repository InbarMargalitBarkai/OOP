...
public class Line {
     ...
     * other, checks if there is intersection and returns the point of the intersect.
     */
     ...
     *
     * @param rect - some rectangle to check the intersection with.
    ...
    public Point closestIntersectionToStartOfLine(Rectangle rect) {
        ...
        if (listOfPoints != null) {
            // variable of distance between the start point and the first point
            double closestDis = this.start().distance(listOfPoints.get(0));
            /*
            creating a variable with the first point in the list,
            so we will check if it is the closest intersection point,
            by checking this with the closest variable, or we need to look for some other point in the list
             */
            Point closestPoint = listOfPoints.get(0);
            for (int i = 0; i < listOfPoints.size(); i++) {
                if (closestDis > this.start().distance(listOfPoints.get(i))) {
                    // changing the distance into the correct one, so we will compare the next iteration with him
                    closestDis = this.start().distance(listOfPoints.get(i));
                    // changing the point into the correct one
                    closestPoint = listOfPoints.get(i);
                }
            }
            return closestPoint;
        }
